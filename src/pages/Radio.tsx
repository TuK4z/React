...
import PowerGoldLogo from '../assets/radio/pwrgold.png';
        ...
        <div className="h-screen bg-custom-gradient">
            <div className="bg-[#0e041c90] h-screen flex items-center justify-center">
                ...
                <div className="relative flex flex-col justify-center items-center">
                    <div
                        className="absolute rounded-full w-32 h-32 flex justify-center items-center text-black z-10  "
                        onClick={TrunOffRadio}
                    >
                        <img className="h-14 transition-all cursor-pointer hover:scale-110" src={TurnOff} alt="Turn Off" />
                    </div>
                    ...
                    <div className="relative flex justify-center items-center" style={{ width: radius * 2, height: radius * 2 }}>
                        {radioStations.map((station, index) => {
                            ...
                            return (
                                <div
                                    key={station.Id}
                                    className="absolute rounded-full transition-all cursor-pointer hover:bg-black bg-opacity-30 w-40 aspect-square flex justify-center items-center text-black group"
                                    style={{ transform: `translate(${x}px, ${y}px)` }}
                                    onClick={() => handleRadio(station)}
                                >
                                    <img className="w-1/2 transition-all group-hover:scale-110" src={station.Image} alt={station.Name} />
                                </div>
                            );
                        })}
                    </div>
